...
<body>
	...
	<script>
		var margin = {
			...
			bottom : 350,
			left : 480
		}, radius = Math.min(margin.top, margin.right, margin.bottom,
				margin.left) - 10;

		var hue = d3.scale.category10();

		var luminance = d3.scale.sqrt().domain([ 0, 1e6 ]).clamp(true).range(
				[ 90, 20 ]);

		var svg = d3.select("body").append("svg").attr("width",
				margin.left + margin.right).attr("height",
		...
		d3.json("<%=cp%>/resources/test.json", function(error, root) {
			...
			var center = svg.append("circle").attr("r", radius / 3).on("click",
					zoomOut);

			center.append("title").text("zoom out");

			var path = svg.selectAll("path").data(
					partition.nodes(root).slice(1)).enter().append("path")
			...
			function zoom(root, p) {
				...
				var enterArc, exitArc, outsideAngle = d3.scale.linear().domain(
						[ 0, 2 * Math.PI ]);
				...
				if (root === p)
					enterArc = outsideArc, exitArc = insideArc, outsideAngle
							.range([ p.x, p.x + p.dx ]);
				...
				if (root !== p)
					enterArc = insideArc, exitArc = outsideArc, outsideAngle
							.range([ p.x, p.x + p.dx ]);

				d3.transition().duration(d3.event.altKey ? 7500 : 750).each(
						function() {
							path.exit().transition().style(
									"fill-opacity",
									function(d) {
										return d.depth === 1 + (root === p) ? 1
												: 0;
							...
							path.transition().style("fill-opacity", 1)
									.attrTween(
											"d",
											function(d) {
